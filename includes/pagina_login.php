<?php
/**
 * Ventana de ingreso SIGAD (la usan index.php y login.php).
 * GET: formulario. POST: valida usuario.
 * Si MySQL falla o faltan archivos, se muestra el aviso aquí (nunca HTTP 500).
 */
$err = '';
$csrfHtml = '';
$mostrarInstalador = false;

try {
    $cfg = dirname(__DIR__) . '/config/config.php';
    if (!is_file($cfg)) {
        $err = 'Falta config/config.php. Abra install.php y vuelva a grabar los datos de MySQL de cPanel (la base ya creada no se borra).';
        $mostrarInstalador = true;
    } else {
        require_once $cfg;
    }
} catch (Throwable $e) {
    error_log('SIGAD login config: ' . $e->getMessage());
    $err = 'No se pudo cargar la configuración. Extraiga el ZIP completo en la carpeta del subdominio y abra install.php.';
    $mostrarInstalador = true;
}
    ...
    try {
        if (function_exists('csrf_check') && !csrf_check()) {
            ...
            if ($user === '' || $pass === '') {
                ...
                $err = 'Falta la conexión a MySQL. Abra install.php y use el usuario, la clave y el nombre exactos de la BD.';
                $mostrarInstalador = true;
        ...
        $err = 'No se pudo conectar a MySQL. Abra install.php y vuelva a indicar usuario, clave y nombre de la BD (cPanel → MySQL Databases). Servidor: localhost. Los expedientes no se borran.';
        $mostrarInstalador = true;
    }
}
...
if (isset($_GET['e']) && $err === '') {
    ...
    if ($cod === 4) $mostrarInstalador = true;
}
...
<div class="login-wrap">
  ...
  <div class="login-box">
    ...
    <?php endif; ?>
    <?php if ($mostrarInstalador): ?>
      <p class="ayuda" style="text-align:center"><a href="install.php" style="color:#1e40af;font-weight:700">Abrir install.php y reparar la conexión</a></p>
    <?php endif; ?>
